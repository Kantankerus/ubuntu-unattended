...
cd $tmp/iso_new
(mkisofs -D -r -V "OTS_UBUNTU" -cache-inodes -J -l -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e boot/grub/efi.img -o $tmp/$new_iso_name . > /dev/null 2>&1) &
spinner $!
